...
- **Case-insensitive DEVELOP/ARCHIVE container names** — container directory names are environment-specific and the casing observed on disk varies between trees, but they were matched case-sensitively against the configured names. A tree spelling its containers `develop`/`archive` under the default `DEVELOP`/`ARCHIVE` config had them walked as ordinary folders, so archived versions and other users' checkouts were indexed as scripts. Names now match case-insensitively in both the script scanner and `scan_checkouts`.
