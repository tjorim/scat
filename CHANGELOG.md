...
- **Catalog stats view** ([#75](https://github.com/tjorim/scat/issues/75)) — `s` opens a new full-screen view visualizing `scat catalog stats` instead of reading its text tables, across three pages (`Left`/`Right` or `h`/`l` to cycle): **Overview** (by-language, by-owner, top tags, most functions per script), **Checkouts & Size** (checkout OS-flavor, most-active checkout users, checkout-staleness and script-size histograms), and **Activity & Audit** (a GitHub-contributions-style checkout-activity calendar heatmap, plus audit findings grouped by check). Fetched by a new one-shot `StatsWorker` (same request/response-channel shape as the existing detail/diff/file-check workers) so opening it never blocks the render loop; each open re-fetches rather than caching indefinitely. Ranking charts cap at the top 8 entries by count, noting how many were cut; the two histograms never truncate, since every bucket (including zero-count ones) is part of the shape being shown. `Esc`/`Backspace` returns to Browse.
  - `scat catalog stats` itself (and its JSON output) also gained these fields directly: `checkout_by_os`, `most_active_checkout_users`, `size_histogram`, `checkout_staleness_histogram`, `findings_by_check`, and `checkout_activity_by_day` (the CLI prints the last as a one-line summary rather than a full day-by-day table). The three checkout-derived fields are cleared when vc isn't configured, matching how `revisions` is already gated, so a former vc config doesn't leave stale data visible.
  - `findings_by_check` runs `scat catalog audit`'s checks internally with default thresholds and no embeddings sidecar, so `near-duplicates`/`outliers` are silently absent from it the same way an unselected `--check` run without a sidecar already omits them — it's a glance at which checks are noisiest, not a substitute for running `scat catalog audit` itself.
- **Dependency graph view** ([#75](https://github.com/tjorim/scat/issues/75)) — `d` opens a full-screen `Canvas`-rendered node-link diagram of the selected script's immediate dependency neighborhood: its "uses" (outbound) and "used by" (inbound) edges, the same data the Deps pane already lists as text, drawn spatially with the center node's edges fanning out left/right. Deliberately scoped to one script's neighborhood rather than the whole catalog — a full-catalog graph of a few thousand scripts would be an unreadable hairball in a terminal — and reuses data the detail worker already loads for the selected script, so it needs no new query or worker. `j`/`k` moves the highlighted node (shared with the Deps pane's own selection), `Enter` re-centers the graph on it (via the same navigation the Deps pane's `Enter` uses), and `[` steps back through the same dependency backstack Deps-pane navigation already maintains.
...
- **`scat-embed`** — new `crates/scat-embed` workspace member: a standalone CLI that reads a `scripts.sqlite` catalog copy read-only, generates embeddings for each script's `logical_path`/`purpose`/`tags`/`owner`/`content` via a local ONNX model (`fastembed`, defaulting to `jina-embeddings-v2-base-code`), and writes them to a sidecar `embeddings.sqlite` (`script_embeddings` table, keyed by `script_id`). Deliberately kept out of the `scat` binary and doesn't depend on `scat_core`, which stays Unix-only — `scat-embed` is a plain cross-platform crate meant to run wherever is convenient (e.g. a laptop with internet access) to produce a sidecar that gets published next to the catalog for the (still offline, Linux-only) `scat` clients to consume.
  - `content` is truncated to a character budget sized against the default model's 8192-token window before embedding, so an oversized script doesn't blow up embedding time/memory; the tokenizer would truncate it anyway once past its own context limit.
  - The sidecar is written to a scratch path beside the destination and published with a single `rename(2)`, so a reader on the shared drive never observes a `script_embeddings` table that exists but is still empty mid-write.
- **`scat similar`** ([#73](https://github.com/tjorim/scat/issues/73)) — new command that ranks scripts already in the catalog by cosine similarity to one script's embedding, reading the `embeddings.sqlite` sidecar `scat-embed` publishes. Brute-force scan over the sidecar's vectors (one row per cataloged script, so no vector-index dependency is needed). This only ever compares two already-embedded scripts — it does not answer free-text queries, since that would require embedding the typed query at search time with the same model, which would pull a heavy ONNX runtime dependency into `scat_core` and conflicts with its offline/lean design goal; that question is still open. Reports a coverage warning when the sidecar covers fewer scripts than the catalog (scripts added since the last `scat-embed` run) and a clear error when the target script has no stored embedding; a stale sidecar entry for a script since removed from the catalog is silently excluded from the ranked results rather than sent to the user as a dead link.
  - Sidecar location is configurable like `db_path`: `--embeddings` / `SCAT_EMBEDDINGS` / `embeddings_path` in the config file, defaulting to `embeddings.sqlite` next to the catalog.
  - Read through the same host-local tmpfs cache as the catalog (`core::cache::catalog_read_path`, generalized beyond the catalog specifically — it has no `index_metadata.build_timestamp` to short-circuit an identity change that still carries the same build, so a changed sidecar is always recopied rather than sometimes re-stamped in place, a fine tradeoff for a file this much smaller than the catalog).
- **`near-duplicates`/`outliers` audit checks and a `most-depended-upon` stats ranking** ([#75](https://github.com/tjorim/scat/issues/75)) — three more consumers of the `embeddings.sqlite` sidecar and the existing dependency graph, alongside `scat similar`:
  - `scat catalog audit --check near-duplicates` scans every pair of embedded scripts (not just relative to one anchor) for cosine similarity at or above `--near-duplicate-threshold` (default `0.97`), flagging likely dedup/consolidation candidates.
  - `scat catalog audit --check outliers` flags scripts whose *best* match among all others still falls below `--outlier-threshold` (default `0.3`) — the catalog's genuinely one-of-a-kind scripts.
  - Both checks are quadratic over the sidecar's row count, in keeping with `EmbeddingsSidecar` already holding every vector in memory; they're skipped silently when no sidecar is available unless explicitly requested via `--check`, in which case `audit` returns a validation error instead.
  - `scat catalog stats` gains three more rankings (top 10 each, `STATS_RANKING_LIMIT`), none needing embeddings: `most_depended_upon` (dependent/in-degree count, from `dependencies` — a blast-radius view of which scripts the most other scripts rely on), `top_tags` (frequency across `scripts.tags`), and `most_functions` (function count per script, from `function_definitions` — a complexity/sprawl view).
