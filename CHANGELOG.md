...
- **Catalog stats view** ([#75](https://github.com/tjorim/scat/issues/75)) — `s` opens a new full-screen view with four of the rankings `scat catalog stats` already computes (by-language, by-owner, top tags, most functions per script), rendered as a 2x2 grid of horizontal bar charts instead of text tables — a direct visual restatement, no new data source. Fetched by a new one-shot `StatsWorker` (same request/response-channel shape as the existing detail/diff/file-check workers) so opening it never blocks the render loop; each open re-fetches rather than caching indefinitely, so a long-lived TUI session reflects the current catalog build. Each chart is capped at the top 8 entries by count, noting how many were cut, so a catalog with many owners still renders legibly in a quarter-screen pane; the two path-labeled charts left-truncate long logical paths to fit. `Esc`/`Backspace` returns to Browse.
