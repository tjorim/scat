...
- **Catalog stats view** ([#75](https://github.com/tjorim/scat/issues/75)) — `s` opens a new full-screen view with the by-language and by-owner counts `scat catalog stats` already computes, rendered as horizontal bar charts side by side instead of text tables — a direct visual restatement, no new data source. Fetched by a new one-shot `StatsWorker` (same request/response-channel shape as the existing detail/diff/file-check workers) so opening it never blocks the render loop; each open re-fetches rather than caching indefinitely, so a long-lived TUI session reflects the current catalog build. Each chart is capped at the top 12 entries by count, noting how many were cut, so a catalog with many owners still renders legibly on one screen. `Esc`/`Backspace` returns to Browse.
...
- **`near-duplicates`/`outliers` audit checks and a `most-depended-upon` stats ranking** ([#75](https://github.com/tjorim/scat/issues/75)) — three more consumers of the `embeddings.sqlite` sidecar and the existing dependency graph, alongside `scat similar`:
  ...
  - `scat catalog stats` gains a `most_depended_upon` ranking (top 10 scripts by dependent/in-degree count) computed from the existing `dependencies` table — no embeddings involved, a blast-radius view of which scripts the most other scripts rely on.
